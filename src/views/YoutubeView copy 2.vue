    ...
    <section class="youtube__btns">
      <div class="container">
        <button
          type="button"
          class="btn"
          v-for="youtubeTag in youtubeTags"
          :key="youtubeTag"
          v-on:click="clickTest(youtubeTag)"
        >
          {{ youtubeTag }}
        </button>
      </div>
...
export default {
  ...
  setup() {
    ...
    const SearchYoutube = () => {
      fetch(
        `https://www.googleapis.com/youtube/v3/search?key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&q=${search.value}&part=snippet&maxResults=30&type=video`
      )
    ...
    const SliderYoutube = () => {
      fetch(
        "https://www.googleapis.com/youtube/v3/search?key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&q=이바다&part=snippet&maxResults=10&type=video"
      )
        .then((response) => response.json())
        // .then((result) => console.log(result.items))
