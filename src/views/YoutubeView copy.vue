...
export default {
  ...
  setup() {
    ...
    const SearchYoutubes = (e) => {
      fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${search.value}&key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&maxResults=30&type=video`
      )
    ...
    const RandomYoutubes = () => {
      fetch(
        "https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=palantir&key=AIzaSyCNlEvsy-8VtD9dAUQihHV-OxP2E9wsOn0&maxResults=30&type=video"
      )
        .then((response) => response.json())
